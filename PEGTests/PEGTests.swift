...
class LiteralTests: XCTestCase {
    func testLiteral() {
        let source = "aaa..."
        eq(Literal("aaa").match(source), Node(source, 0..<3))
    }
    
    func testLiteralSuccessfulMatchNotFromBeginningOfString() {
        let source = "...aaa..."
        eq(Literal("aaa").match(source, at: 3), Node(source, 3..<6))
    }
    
    func testLiteralUnsuccessfulMatch() {
        let source = "..."
        XCTAssertNil(Literal("aaa").match(source))
    }
}
